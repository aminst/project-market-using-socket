...
void send_result_to_server(int client_fd, int price, int project_number)
{
    char result[20] = {0};
    strcat(result, "$");
    strcat(result, int_to_str(project_number));
    strcat(result, "$");
    strcat(result, int_to_str(price));
    send(client_fd, result, sizeof(result), 0);
}

void alarm_handler(int num)
{
    const char *time_passed_msg = "10 Secons Passed Without Input!\n";
    write(1, time_passed_msg, strlen(time_passed_msg));
}
...
void run_project_market(int my_id, int udp_fd, struct sockaddr_in udp_sin, int prj_id, int port)
{
    ...
    while (count != 4)
    {
        ...
        else
        {
            ...
            int price;
            if (strcmp(price_str, "time") == 0)
            {
                const char *timeout_msg = "Player Timed Out!\n";
    ...
    if (my_id == winner)
        send_result_to_server(client_fd, min_price, prj_id);
}
...
void run_client(int port)
{
    ...
    int project_id = send_selected_project_number_to_server(client_fd);
    ...
    run_project_market(atoi(id_str), udp_socket_fd, udp_sin, project_id, port);
